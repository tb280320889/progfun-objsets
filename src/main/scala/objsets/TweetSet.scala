...
abstract class TweetSet {
	 ...
	 * and be implemented in the subclasses?
	 */
	def union(that: TweetSet): TweetSet = filterAcc(p => this.contains(p), that.filterAcc(p => that.contains(p), new Empty))
	 ...
	 * and be implemented in the subclasses?
	 */
	def mostRetweeted: Tweet = mostAcc(new Tweet("", "", 0))
...
class Empty extends TweetSet {
	...
	override def mostAcc(tweet: Tweet): Tweet = tweet
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
	...
	def mostAcc(x: Tweet): Tweet = {
		if (x.retweets > elem.retweets)
			right.mostAcc(left.mostAcc(x))
